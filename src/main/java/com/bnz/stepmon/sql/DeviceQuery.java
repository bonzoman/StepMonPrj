...
public interface DeviceQuery {
    ...
    @Insert("""
            INSERT INTO device_registration
              (install_id, device_token, is_notification_enabled, app_version, last_push_at,
               push_fail_count, is_active, deactivated_at, deactivated_reason, first_seen_at, last_seen_at)
            VALUES
              (#{installId}, #{deviceToken}, #{isNotificationEnabled}, #{appVersion},NULL,
               0, 1, NULL, NULL, UTC_TIMESTAMP(3), UTC_TIMESTAMP(3))
    ...
    @Select("""
                ...
                WHERE install_id = #{installId}
            """)
    DeviceResDto findById(@Param("installId") String installId);
