...
function introHeader(){
    if($('.pagecnd1').length>0 || ($('.pagecnd2').length>0 && $('.banner-box1').length>0) || $('.pagecnd4').length>0){
        $(window).scrollTop(0);
        $('header').removeClass('active')
        $(window).scroll(function(){
            if($(window).scrollTop()+$('header').outerHeight()>$('.container>div:first').next('div:not(.pop-mes4)').offset().top){
